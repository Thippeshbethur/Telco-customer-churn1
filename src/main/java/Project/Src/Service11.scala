...
object Service11 {
  def main(args: Array[String]): Unit = {
    ...
    service.createOrReplaceTempView("service_table")
    sparkSession.sql("select customerID from service_table where gender=''")
      .coalesce(1).write.option("header", "true")
      .csv("C:\\Users\\Thipu\\Desktop\\Badrecords")
    sparkSession.sql("select count(customerID) from service_table where gender=''").show()
    var df = sparkSession.sql("SELECT " +
    ...
    df.coalesce(1).write.option("header", "true")
      .csv("C:\\Users\\Thipu\\Desktop\\sample_fi")
  }
}
